      ...
      <div>
        <el-form label-width="80px">
          ...
          <el-form-item label="包装费">
            <el-input-number v-model="product.packprice" :min="0" :max="10" label="包装费"></el-input-number>
          </el-form-item>
          <el-form-item label="价格">
            <el-input-number v-model="product.price" :min="0" :step="1" :max="200" label="价格"></el-input-number>
          </el-form-item>
...
export default {
  ...
  created() {
    ...
    this.$axios.get(`goods.php?a=readOne&id=${id}`).then(data => {
      this.product = data.data.data;
      this.product.goodsfeature = data.data.data.goodsfeature.split(",");
    });
  },
  methods: {
    handleAvatarSuccess(res) {
      this.product.imgUrl = "http://m.zcblack.top/sell/" + res.data;
    },
    beforeAvatarUpload() {},
    onSubmit() {
      ...
      this.$axios
        .post(
          "goods.php?a=edit",
          `id=${data.id}&goodsname=${data.goodsname}&goodscategory=${data.goodscategory}&goodsfeature=${data.goodsfeature}&imgUrl=${data.imgUrl}
    ...
    cancel() {
      this.$router.push("/index/admin_product-list");
    }
  }
};
</script>
...
#add-product .avatar-uploader .el-upload {
...
#add-product .avatar-uploader .el-upload:hover {
  border-color: #409eff;
}
#add-product .avatar-uploader-icon {
  font-size: 28px;
  color: #8c939d;
  width: 148px;
  height: 148px;
